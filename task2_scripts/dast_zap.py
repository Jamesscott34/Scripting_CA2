...
def main() -> None:
  ...
  if args.auto:
    # Derive a base name of the form zap_<host>_<ddmmyy>. We keep ZAP's native
    # multi-format reports under logs/zap_reports, but also ensure that JSON,
    # HTML, Excel, and text logs are mirrored into the same logs/ hierarchy
    # used by the other tooling (json_logs, excel, html, root logs).
    parsed = urlparse(args.target)
    host = parsed.netloc or parsed.path or "target"
    safe_host = host.replace(":", "_").replace(".", "_")
    date_str = datetime.now().strftime("%d%m%y")
    base_name = f"zap_{safe_host}_{date_str}"

    # Always keep a canonical ZAP report prefix under logs/zap_reports for the
    # raw HTML / Markdown / Excel outputs that come directly from ZAP or this
    # helper.
    if output_base is None:
      output_base = LOGS_ROOT / "zap_reports" / base_name

    # In auto mode we prefer JSON to live under logs/json_logs alongside the
    # other tools, so we wire --output-json explicitly and let formats drive
    # only the additional artefacts (HTML/MD/XLSX).
    if output_json is None:
      json_dir = LOGS_ROOT / "json_logs"
      json_dir.mkdir(parents=True, exist_ok=True)
      output_json = json_dir / f"{base_name}.json"

    if not formats:
      # Default to the human-friendly formats; JSON is already handled via
      # --output-json above to avoid duplicate files.
      formats = ["html", "md", "xlsx"]
  ...
  if result is not None:
    # In auto mode, ensure key artefacts are mirrored into the standard logs/
    # subdirectories used across the project so that reports are easy to find.
    if args.auto and not args.summary_only and output_base is not None:
      try:
        # Mirror the canonical JSON (which already lives under logs/json_logs
        # in auto mode) into that directory, and place HTML/Excel/text logs
        # into their respective logs/ subfolders if they were generated.
        base = output_base

        # HTML report -> logs/html
        html_src = base.with_suffix(".html")
        if html_src.exists():
          html_dir = LOGS_ROOT / "html"
          html_dir.mkdir(parents=True, exist_ok=True)
          html_dst = html_dir / html_src.name
          html_dst.write_text(html_src.read_text(encoding="utf-8"), encoding="utf-8")

        # Excel report -> logs/excel
        xlsx_src = base.with_suffix(".xlsx")
        if xlsx_src.exists():
          excel_dir = LOGS_ROOT / "excel"
          excel_dir.mkdir(parents=True, exist_ok=True)
          xlsx_dst = excel_dir / xlsx_src.name
          # Openpyxl writes binary files; preserve that here as bytes.
          xlsx_dst.write_bytes(xlsx_src.read_bytes())

        # Text log -> logs root (mirroring Bandit/fuzz style).
        log_src = base.with_suffix(".log")
        if log_src.exists():
          LOGS_ROOT.mkdir(parents=True, exist_ok=True)
          log_dst = LOGS_ROOT / log_src.name
